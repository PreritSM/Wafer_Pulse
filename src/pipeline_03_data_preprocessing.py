...
def missing_value_imputation(df, config):
    ...
    logger.info(
        "Mean imputation complete. Missing before=%d, after=%d",
        missing_values_bfr_imputation,
        missing_values_aft_imputation,
    )
    with open(
        os.path.join(config["artifacts_dir"]["general"], "Missing_value_processing.txt"), "a"
    ) as f:
...
def missing_value_examination(config):
    ...
    with open(
        os.path.join(config["artifacts_dir"]["general"], "Missing_value_processing.txt"), "w"
    ) as f:
        f.write(
            f"Total Missing Values: {total_missing} out of {total_cells} cells ({missing_percentage:.2f}%)\n"
        )
        f.write(
            f"\nMissing Values per Column: {len(cols_with_missing)} columns with missing values\n"
        )
        f.write(
            f"Columns without Missing Values: {df.shape[1] - len(cols_with_missing)} columns\n"
        )
    ...
    logger.info(
        "Dropped %d columns exceeding %.0f%% missing threshold",
        len(cols_to_drop),
